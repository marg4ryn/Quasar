<template>
  ...
  <div class="auth-page">
    <div class="auth-container">
      ...
      <form @submit.prevent="handleRegister" class="auth-form">
        ...
        <div class="form-group">
          ...
          <input
            ...
            :disabled="loading"
          />
          <small class="form-hint">{{ t('auth.passwordHint') }}</small>
        </div>
      ...
      <button @click="handleGoogleLogin" type="button" class="google-btn" :disabled="loading">
...
<script setup lang="ts">
  ...
  const handleRegister = async () => {
    ...
    if (password.value !== confirmPassword.value) {
      error.value = t('auth.errorPasswordsMismatch')
      return
    }

    if (password.value.length < 6) {
      error.value = t('auth.errorPasswordTooShort')
      return
    }
    ...
    try {
      ...
      error.value = err?.message || t('auth.errorRegisterFailed')
    } finally {
      loading.value = false
    }
  }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err?.message) {
        ...
        error.value = t('auth.googleLoginError')
      }
    } finally {
      loading.value = false
    }
